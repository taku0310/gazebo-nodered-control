...
## gz transport（Gazebo Fortress 内部）

| トピック | 型 | 方向 | 発行元 | 受信先 |
|---|---|---|---|---|
| `/cmd_vel` | `ignition.msgs.Twist`    | bridge → sim | `ros_gz_bridge` | `ignition::gazebo::systems::DiffDrive` |
| `/odom`    | `ignition.msgs.Odometry` | sim → bridge | `ignition::gazebo::systems::DiffDrive` | `ros_gz_bridge` |

これらは gazebo コンテナ内部の `ros_gz_bridge` 経由でのみ翻訳されます。
MQTT クライアントや SoftPLC から gz transport を意識する必要はありません。
Harmonic に乗り換える場合は `ignition.msgs.*` を `gz.msgs.*` に書き換える
だけで済みます（ROS 側の契約は不変）。
...
|---|---|---|
| `/cmd_vel` `geometry_msgs/Twist` | `/cmd_vel` `ignition.msgs.Twist` | `ros_gz_bridge` がフィールドを 1:1 マップ |
| `/odom` `nav_msgs/Odometry` | `/odom` `ignition.msgs.Odometry` | 同上 |
