...
├── mosquitto/                  # ブローカー設定（→ [サービス詳細](mosquitto/README.md)）
│   └── config/
├── nodered/                    # Dashboard（→ [サービス詳細](nodered/README.md)）
│   ├── Dockerfile              # Node-RED + node-red-dashboard
│   └── flows.json              # インポート可能な Dashboard フロー
├── ros2_bridge/                # ブリッジ（→ [サービス詳細](ros2_bridge/README.md)）
│   ├── Dockerfile              # ros:humble + paho-mqtt + ワークスペース
│   └── ws/src/mqtt_ros2_bridge # ブリッジノードパッケージ
└── gazebo/                     # シミュ + noVNC（→ [サービス詳細](gazebo/README.md)）
...
各サービスのフォルダ直下に **個別 README** を置いてあります。役割 /
処理内容 / コンポーネント図 / シーケンス図 / アクティビティ図 / 状態
遷移図 / ユースケース図がすべて Mermaid で入っています:

* [`gazebo/README.md`](gazebo/README.md) — Gazebo Fortress + ros_gz_bridge + noVNC
* [`mosquitto/README.md`](mosquitto/README.md) — MQTT ブローカー、retained、LWT
* [`nodered/README.md`](nodered/README.md) — Dashboard フローとノード構成
* [`ros2_bridge/README.md`](ros2_bridge/README.md) — ブリッジノードの内部設計
