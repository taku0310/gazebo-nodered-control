...
└── gazebo/
    ├── Dockerfile              # ros:humble + Gazebo Harmonic + Xvfb/x11vnc/noVNC
    ├── entrypoint.sh           # Xvfb / x11vnc / websockify を起動
    ├── launch/gazebo.launch.py # gz sim + ros_gz_bridge
    ├── config/ros_gz_bridge.yaml # /cmd_vel と /odom のブリッジ定義
    └── worlds/diff_drive.sdf   # SDF 1.10 + gz-sim-diff-drive-system
```
...
Gazebo Harmonic の GUI はコンテナ内の Xvfb（仮想ディスプレイ）に出力され、
...
| ROS 2 Jazzy + Gazebo Harmonic | ROS 2 Humble + Gazebo Harmonic（`ros_gz_bridge` 経由） |
