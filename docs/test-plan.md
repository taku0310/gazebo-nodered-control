...
3D ビューは `http://127.0.0.1:8080/vnc.html`（「Connect」をクリック）、
ダッシュボードは `http://127.0.0.1:1880/ui` で開けます。
...
| 手順 | 操作 | 期待結果 |
|---|---|---|
| 1 | `http://127.0.0.1:1880/ui` を開く | DRIVE / SPEED / TELEMETRY の各グループが表示される。BRIDGE バッジが緑の `ONLINE` で脈動 |
| 2 | SPEED スライダーを 1.0 に設定 | 波形ゲージが 1.00 を指す。まだロボットは動かない |
| 3 | **FORWARD** ボタンをクリック | `robot/cmd_vel` に `{"linear_x":1,"angular_z":0}` が流れる。3D ビューでロボットが前進 |
| 4 | TELEMETRY を確認 | `POSE X` が増加、`LIN.X ≈ 1.00 m/s`、`ANG.Z ≈ 0 rad/s` |
| 5 | **STOP** をクリック | 減速して停止。`LIN.X` → 0。約 1 秒後に `LATEST ALARM` カードに `WATCHDOG_TIMEOUT` が出る |
| 6 | **LEFT** → **RIGHT** をクリック | `ANG.Z` の符号が反転、`LIN.X` = 0。アラームカードが `WATCHDOG_CLEAR`（緑）に切替 |
| 7 | **BACKWARD** をクリック | `LIN.X` = -1、ロボットが後退 |
